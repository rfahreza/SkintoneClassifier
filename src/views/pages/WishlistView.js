  ...
  async render() {
    ...
    if (!user) {
      return await Layout.wrap(`
        <section class="py-5 bg-light" style="min-height:80vh; margin-top:50px;">
        ...
        <div class="container">
          ...
          ${wishlist.length > 0 ? `
              ...
              ${wishlist.map(product => `
                <div class="col-md-6 col-lg-4">
                  <div class="card beauty-card p-3 h-100 position-relative">
                    <button class="btn btn-danger btn-sm position-absolute top-0 end-0 m-2 rounded-circle remove-wishlist-btn" data-id="${product.id}" title="Hapus dari wishlist"><i class="bi bi-trash"></i></button>
                    <img src="${product.image}" alt="${product.name}" class="card-img-top rounded-3 mb-3" style="height:220px;object-fit:cover;">
                    <span class="position-absolute bottom-0 start-0 mb-3 ms-3 px-2 py-1 rounded-pill small ${product.platform === 'shopee' ? 'bg-warning text-dark' : product.platform === 'sociolla' ? 'bg-pink text-white' : 'bg-dark text-white'}">
                      ${product.platform === 'shopee' ? 'Shopee' : product.platform === 'sociolla' ? 'Sociolla' : 'Zalora'}
                    </span>
                    <div class="card-body p-0">
                      ...
                      <div class="mb-2 d-flex align-items-center justify-content-between">
                        ...
                        <span class="badge rounded-pill ${product.undertoneMatch === 'warm' ? 'bg-warning text-dark' : product.undertoneMatch === 'cool' ? 'bg-primary' : product.undertoneMatch === 'neutral' ? 'bg-success' : 'bg-secondary'}">
                          ${product.undertoneMatch === 'all' ? 'All' : product.undertoneMatch}
                        </span>
                      </div>
                      <p class="card-text small mb-2">${product.description}</p>
                      <a href="${product.shopLink}" target="_blank" rel="noopener" class="btn btn-pink w-100 mt-2 d-flex align-items-center justify-content-center gap-2">
            ...
            <div class="mt-5 card beauty-card p-4 text-center">
              ...
              <div class="fs-2 fw-bold text-danger mb-2">Rp ${wishlist.reduce((total, p) => total + (p.price || 0), 0).toLocaleString('id-ID')}</div>
          ...
          ${user.undertoneResult ? `
            <div class="mt-5 bg-gradient-pink rounded-4 p-5 text-center text-white">
              <h3 class="fw-bold mb-3">Rekomendasi Khusus untuk Undertone ${user.undertoneResult.type}</h3>
              <p class="mb-4">Temukan produk lainnya yang cocok dengan undertone kulitmu</p>
              <a href="/products?undertone=${user.undertoneResult.type}" class="btn btn-light text-danger fw-semibold px-4 py-2 rounded-pill d-inline-flex align-items-center gap-2 shadow-sm">
